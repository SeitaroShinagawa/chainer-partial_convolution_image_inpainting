...
class PConv(chainer.Chain):
    def __init__(self, ch0, ch1, bn=True, sample='none-3', activation=F.relu, dropout=False, noise=False):
        ...
        if sample=='down-4':
            ...
            layers['m'] = L.Convolution2D(ch0, ch1, 4, 2, 1, initialW=1.0, nobias=True)
        elif sample=='down-8':
            layers['c'] = L.Convolution2D(ch0, ch1, 8, 2, 3, initialW=w, nobias=True)
            layers['m'] = L.Convolution2D(ch0, ch1, 8, 2, 3, initialW=1.0, nobias=True)
        elif sample=='down-2':
            layers['c'] = L.Convolution2D(ch0, ch1, 2, 2, 0, initialW=w, nobias=True)
            layers['m'] = L.Convolution2D(ch0, ch1, 2, 2, 0, initialW=1.0, nobias=True)
        else:
